...
const dbConfig = {
  ...
  database: process.env.DB_NAME || 'perpustakaan_kampus',
...
async function createTables() {
  ...
  // Create search index on buku(judul) if it doesn't exist
  try {
    const [indexes] = await pool.query(`SHOW INDEX FROM buku WHERE Key_name = 'idx_buku_judul';`);
    if (indexes.length === 0) {
